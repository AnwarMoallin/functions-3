...
// CODE HERE
const newNumbers = numbers.map((number) => console.log(number / 2));
console.log("----")
...
// CODE HERE
let cityEndsWithI = cities.filter(city => {
  if(city.slice(-1) === "i"){
    console.log(city)
  }
})
...
fruits.forEach((item) => {
  if (item.color === "red") {
    console.log(`The fruit with index ${fruits.indexOf(item)} is an apple`);
  } else if (item.color === "orange") {
    console.log(`The fruit with index ${fruits.indexOf(item)} is an orange`);
  } else {
    console.log(`The fruit with index ${fruits.indexOf(item)} is neither apple or orange`);
  }
});
...
// CODE HERE
let totalRicePrice = foods.filter((item) => item.tags.includes("rice")).reduce((adder, item) => adder + item.price, 0);
